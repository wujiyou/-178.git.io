    ...
    myAddEvent(document,'scroll',function(){
        ...
        for (var index = 0; index < elements.length; index++) {
            ...
            if(elTop*1.5<=clientHeight){
                el.className = 'row w-row animated '+animate; 
                el.style['animation-delay']=n +"ms";
            }else{
                el.className = 'row w-row';
            }
...
myAddEvent(window,'load',function(){
    // 创建DIV节点
    // var oBody = document.getElementsByTagName('parter-content')[0];
    ...
    // }
    // // 执行可视区函数
    visibleAnimate(document.getElementsByClassName('w-row'))
})